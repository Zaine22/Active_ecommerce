...
class SendSMSUtility
{
    public static function sendSMS($to, $from, $text, $template_id)
    {
        if (OtpConfiguration::where('type', 'nexmo')->first()->value == 1) {
            ...
            $api_key = env("NEXMO_KEY"); // Your SMSPoh API key
            $sender_id = env("NEXMO_SENDER_ID"); // Your SMSPoh Sender ID
            ...
            $params = http_build_query($params); // Format parameters as a query string
            ...
            // Check for success in the SMSPoh response
            if (isset($response['status']) && $response['status'] === 'success') {
                return $response;
            }
            return json_encode(["error" => "Failed to send SMS", "response" => $response]);
        } elseif (OtpConfiguration::where('type', 'smspoh')->first()->value == 1) {
